...
if [ -z "${PARAMETER_OVERRIDES_STRING}" ]; then
  ...
  ${aws_command} cloudformation deploy --stack-name "${STACK_NAME}" --template-file "${stack_file_full_path}" --capabilities CAPABILITY_IAM --parameter-overrides $PARAMETER_OVERRIDES_STRING --no-execute-changeset
fi
if [ $? -ne 0 ]; then
